...
V = [[-1 for _ in range(num_vertex)] for _ in range(num_vertex)] #경유지 저장 -1: 직접간다. 다 직접간다고 써놓음
for s,e,w in edges:
  D[s][e] = w
  D[e][s] = w * 6 // 10
# print(D)
for k in range(num_vertex):
  for s in range(num_vertex):
    ...
    for e in range(num_vertex):
      ...
      if D[s][e] > D[s][k] + D[k][e]: #s에서 e가는데 k들러가는게 더 가까우면
        D[s][e] = D[s][k] + D[k][e] # 그렇게 하자
        V[s][e] = k #s에서 e갈 때는 k를 들러가야한다
